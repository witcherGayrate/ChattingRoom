    ...
    , ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    this->setModal(true);
    ui->name_ed->setPlaceholderText("苗条的胖红");
    ...
    //显示更多标签
    connect(ui->more_lab,&ClickedOnceLabel::clicked,this,&ApplyFriend::ShowMoreLabel);
    InitTipLbs();//初始化好友标签列表
    //连接输入标签回车事件
    connect(ui->lab_ed,&CustomLineEdit::returnPressed,this,&ApplyFriend::SlotLabelEnter);
    connect(ui->lab_ed,&CustomLineEdit::textChanged,this,&ApplyFriend::SlotLabelTextChange);
    connect(ui->lab_ed,&CustomLineEdit::editingFinished,this,&ApplyFriend::SlotLabelEditFinished);
    connect(ui->tip_lab,&ClickedOnceLabel::clicked,this,&ApplyFriend::SlotAddFirendLabelByClickTip);
